...
async function pesanAwal() {
    ...
    await swalst.fire({
        title: 'Chào eiuu nha ❤️',
        imageUrl: '' + stiker1.src,
    });
    await swalst.fire({
        title: 'Anh chỉ muốn nói 😍',
        imageUrl: '' + stiker2.src,
    });
    await swalst.fire({
        title: 'Mỗi lần nghe thấy chuông báo tin nhắn, anh đều hi vọng một trong những tin nhắn đó là của em 🤭❤️',
        imageUrl: '' + stiker3.src,
    });
    await swalst.fire({
        title: 'Dung lượng bộ nhớ điện thoại của anh có thể có hạn 🥺❤️',
        imageUrl: '' + stiker4.src,
    });
    await swalst.fire({
        title: 'Nhưng trái tim anh thì có vô cùng tận khoảng trống dành cho em 😘💕',
